...
export default function MealCard({ meal, index }) {
  ...
  const mainPrompt = meal.image_prompt || meal.title || 'gourmet meal';
  const sidesText = [meal.side, meal.side2].filter(Boolean).join(', ');
  const fullPrompt = sidesText ? `${mainPrompt} with ${sidesText}` : mainPrompt;
  
  const imageUrl = `https://image.pollinations.ai/prompt/gourmet food dish, ${encodeURIComponent(fullPrompt)}?seed=${seed}&width=800&height=600&nologo=true&model=flux`;
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="font-fresh text-3xl text-[#1b4d3e] leading-none mb-2">{meal.title}</h3>
